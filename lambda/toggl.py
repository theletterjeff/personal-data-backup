...
def toggl_handler(start: int, end: int) -> dict[str, Any]:
    ...
    time_entries = [parse_time_entry(t) for t in get_time_entries(start, end, projects, api_key)]
...
def get_time_entries(start: int, end: int, projects: list[Project], api_key: str):
    ...
    resp = requests.get(url, auth=(api_key, "api_token"), headers=headers, params=params)
    return resp.json()
